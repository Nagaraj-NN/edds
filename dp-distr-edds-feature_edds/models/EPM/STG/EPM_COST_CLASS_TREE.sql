...
-- MAPPING NAME      : COST_TREE
-- TARGET TABLE      : EDDSP.EPM.EPM_COST_CLASS_TREE
-- AUTOSYS_JOB_NAME  : LOAD_EDDS_TREE_TABLES_ASYS
-- MATERIALIZATION   : table
-- ============================================================
-- ============================================================
-- CHANGE LOG (2026-09-12)
-- RENAMED : was ADD_MISSING_GLBUS.sql; the file name now matches the table this model builds.
...
SELECT RTRIM(a.tree_node)  child,
 ...
 WHERE c.setid = 'AEP'
   ...
   AND c.tree_name = 'COST_COMPONENT'
...
SELECT RTRIM(a.tree_node)  child,
 ...
 WHERE b.setid = 'AEP'
   ...
   AND b.tree_name = 'COST_COMPONENT'
...
(
    SELECT Q.$1 AS C1, Q.$2 AS C2, Q.$3 AS C3, Q.$4 AS C4, Q.$5 AS C5
    FROM (
SELECT (B.TREE_NODE), (B.DESCR), D.TREE_LEVEL_NUM + 1, (A.RESOURCE_TYPE), (A.DESCR) 

FROM

 {{ source('CRPDB01_EPMADM', 'PS_PROJ_RES_TP_D00') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PS_PROJ_RES_TP_D00 */  A, {{ source('CRPDB01_EPMADM', 'PS_TREE_NODE_TBL') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PS_TREE_NODE_TBL */  B,{{ source('CRPDB01_EPMADM', 'PSTREELEAF') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PSTREELEAF */  C, {{ source('CRPDB01_EPMADM', 'PSTREENODE') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PSTREENODE */  D
...
AND A.RESOURCE_TYPE BETWEEN C.RANGE_FROM AND C.RANGE_TO

AND A.EFFDT = ( SELECT MAX(EFFDT) 

     FROM {{ source('CRPDB01_EPMADM', 'PS_PROJ_RES_TP_D00') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PS_PROJ_RES_TP_D00 */  A1

     WHERE A1.SETID = A.SETID

     AND A1.RESOURCE_TYPE = A.RESOURCE_TYPE)
...
(
    SELECT
        C1 AS CHILD_COST_CLASS_STR,
        C2 AS CHILD_COST_CLASS_DESC,
        C3 AS PARENT_COST_CLASS_STR,
        C4 AS PARENT_COST_CLASS_DESC,
    ...
    SELECT
        C4 AS CHILD_COST_CLASS_STR,
        C5 AS CHILD_COST_CLASS_DESC,
        C1 AS PARENT_COST_CLASS_STR,
        C2 AS PARENT_COST_CLASS_DESC,
...
SELECT
    CAST(CHILD_COST_CLASS_STR AS VARCHAR(20))  AS CHILD_COST_CLASS_STR,
    CAST(CHILD_COST_CLASS_DESC AS VARCHAR(40)) AS CHILD_COST_CLASS_DESC,
    CAST(PARENT_COST_CLASS_STR AS VARCHAR(20)) AS PARENT_COST_CLASS_STR,
    CAST(PARENT_COST_CLASS_DESC AS VARCHAR(40)) AS PARENT_COST_CLASS_DESC,
    CAST(DESCENDANT_LEVEL AS NUMBER(38,18))    AS DESCENDANT_LEVEL,
    CAST(LOAD_DATE AS TIMESTAMP_NTZ(9))        AS LOAD_DATE
FROM FINAL_COST_TREE
