...
-- MAPPING NAME      : DEPARTMENT_TREE
-- TARGET TABLE      : EDDSP.EPM.EPM_GL_DEPT_TREE
-- AUTOSYS_JOB_NAME  : LOAD_EDDS_TREE_TABLES_ASYS
-- MATERIALIZATION   : table
-- ============================================================
-- ============================================================
-- CHANGE LOG (2026-09-12)
-- RENAMED : was EPM_GL_BUSINESS_UNIT_TREE.sql; the file name now matches the table this model builds.
...
SQ_DEPARTMENT_TREE_2 AS
(
    SELECT Q.$1 AS C1, Q.$2 AS C2, Q.$3 AS C3, Q.$4 AS C4, Q.$5 AS C5
    FROM (
SELECT (A.DEPTID), (A.DESCR), (B.TREE_NODE), (B.DESCR), D.TREE_LEVEL_NUM + 1

FROM

 {{ source('CRPDB01_EPMADM', 'PS_DEPARTMENT_TBL') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PS_DEPARTMENT_TBL */  A, {{ source('CRPDB01_EPMADM', 'PS_TREE_NODE_TBL') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PS_TREE_NODE_TBL */  B,{{ source('CRPDB01_EPMADM', 'PSTREELEAF') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PSTREELEAF */  C, {{ source('CRPDB01_EPMADM', 'PSTREENODE') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PSTREENODE */  D
...
AND A.DEPTID BETWEEN C.RANGE_FROM AND C.RANGE_TO

AND A.EFFDT = ( SELECT MAX(EFFDT) 

     FROM {{ source('CRPDB01_EPMADM', 'PS_DEPARTMENT_TBL') }} /* CRPDB01_DEV_SANDBOX.EPMADM.PS_DEPARTMENT_TBL */  A1

     WHERE A1.SETID = A.SETID

     AND A1.DEPTID = A.DEPTID

     AND A1.EFFDT <= C.EFFDT)
...
FINAL_DEPARTMENT_TREE AS
(
    SELECT
        C1 AS CHILD_GL_DEPT_STR,
        C2 AS CHILD_GL_DEPT_DESC,
        C3 AS PARENT_GL_DEPT_STR,
        C4 AS PARENT_GL_DEPT_DESC,
        CAST(C5 AS NUMBER(38,18)) AS DESCENDANT_LEVEL,
        CURRENT_TIMESTAMP() AS LOAD_DATE
    FROM SQ_DEPARTMENT_TREE_1
    ...
    SELECT
        ...
        CAST(C5 AS NUMBER(38,18)) AS DESCENDANT_LEVEL,
        CURRENT_TIMESTAMP() AS LOAD_DATE
    FROM SQ_DEPARTMENT_TREE_2
)

SELECT
    CAST(CHILD_GL_DEPT_STR AS VARCHAR(20))    AS CHILD_GL_DEPT_STR,
    CAST(CHILD_GL_DEPT_DESC AS VARCHAR(40))   AS CHILD_GL_DEPT_DESC,
    CAST(PARENT_GL_DEPT_STR AS VARCHAR(20))   AS PARENT_GL_DEPT_STR,
    CAST(PARENT_GL_DEPT_DESC AS VARCHAR(40))  AS PARENT_GL_DEPT_DESC,
    CAST(DESCENDANT_LEVEL AS NUMBER(38,18))   AS DESCENDANT_LEVEL,
    CAST(LOAD_DATE AS TIMESTAMP_NTZ(9))       AS LOAD_DATE
FROM FINAL_DEPARTMENT_TREE
